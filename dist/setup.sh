...
install_apps() {
  if [ ! -d "$current_path/apps" ]; then
    echo "      FETCHING CYPHERAPPS REPO           "
    local apps_repo="https://github.com/veriphi/cypherapps.git"
    echo "   [32mclone[0m $apps_repo into apps"
    docker run --rm -v "$current_path":/git --entrypoint git cyphernode/cyphernodeconf:$CONF_VERSION clone --single-branch -b master "$apps_repo" /git/apps > /dev/null 2>&1
  fi
}
